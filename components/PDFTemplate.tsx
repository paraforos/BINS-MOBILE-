...
const PDFTemplate: React.FC<PDFTemplateProps> = ({ data, page1Ref, page2Ref }) => {
    ...
    <div className="mb-6">
      <div className="flex justify-between items-end mb-4">
        ...
        <div className="text-right">
          <h1 className="text-[18px] font-black uppercase tracking-tighter leading-none mb-1 text-black">ΑΝΑΦΟΡΑ ΣΠΑΣΜΕΝΩΝ BINS</h1>
        </div>
      </div>
  ...
  const Footer = () => (
    <div className="mt-auto pt-4 border-t border-gray-100 flex justify-between items-center">
      ...
      <p className="text-[7px] text-gray-400 font-medium italic">Sent from Android</p>
    </div>
  );

  const InfoBox = ({ label, value }: { label: string, value: string }) => (
    <div className="border-b border-gray-200 pb-1">
      <p className="text-[8px] font-black text-gray-400 mb-0.5 uppercase tracking-wider">{label}</p>
      <p className="text-[12px] font-bold uppercase text-black">{value || '-'}</p>
    </div>
  );

  const StatBox = ({ label, value }: { label: string, value: string }) => (
    <div className="border border-gray-200 p-4 rounded-xl flex flex-col justify-center bg-white shadow-none">
      <p className="text-[8px] font-black text-gray-400 mb-0.5 uppercase tracking-wider">{label}</p>
      <p className="text-[26px] font-black leading-none text-black">{value || '0'}</p>
    </div>
  );
  ...
  return (
    <div className="fixed -left-[5000px] top-0 pointer-events-none flex flex-col gap-10">
      ...
      <div ref={page1Ref} className="bg-white w-[210mm] h-[297mm] flex flex-col p-[20mm] box-border overflow-hidden text-black font-sans shadow-none border-none">
        ...
        <div className="mb-6 p-4 border border-gray-200 rounded-xl min-h-[60px] bg-white">
          <p className="text-[8px] font-black text-gray-400 mb-1 uppercase tracking-wider">ΣΧΟΛΙΑ / ΠΑΡΑΤΗΡΗΣΕΙΣ</p>
          <p className="text-[10px] font-medium leading-relaxed italic text-black">
            {data.comments || 'Δεν υπάρχουν επιπλέον σχόλια.'}
          </p>
        </div>

        <div className="flex-1">
          <h2 className="text-[9px] font-black uppercase mb-3 tracking-widest border-l-4 border-black pl-3">ΦΩΤΟΓΡΑΦΙΕΣ</h2>
          <div className="grid grid-cols-2 gap-3">
            {firstPagePhotos.map((photo, i) => (
              <div key={i} className="aspect-[3/2] rounded-lg overflow-hidden border border-gray-100 bg-white flex items-center justify-center">
      ...
      {secondPagePhotos.length > 0 && (
        <div ref={page2Ref} className="bg-white w-[210mm] h-[297mm] flex flex-col p-[20mm] box-border overflow-hidden text-black font-sans shadow-none border-none">
          <Header />
          <div className="flex-1">
            <h2 className="text-[9px] font-black uppercase mb-4 tracking-widest border-l-4 border-black pl-3">ΣΥΝΕΧΕΙΑ ΦΩΤΟΓΡΑΦΙΩΝ</h2>
            <div className="grid grid-cols-2 gap-3">
